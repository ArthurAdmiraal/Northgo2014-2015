...
///////////////////////////////////////////////////////////////////////////
// Teleop_compass.c                                                      //
// Simple teleop program for Robot C using the OmniWheelDriver library   //
...
task main() {
	...
	while(true) {
		...
		OWsetDriveVec(x1/(joy1Btn(7)*3+1), y1/(joy1Btn(7)*3+1));
		OWsetRotationSpeed(pow(abs(deadZonei(joystick.joy1_x2, LIMIT)*100/127), 2)*sgn(joystick.joy1_x2)*ROTATION/(10000*(1+9*!joy1Btn(8))));

		servo[servo1] = joy1Btn(6)*80;

		OWsetMode(CMPS_CMP*!joy1Btn(5));

		if(joystick.joy1_TopHat==2) {
			OWadjustCalibration(0.5);
